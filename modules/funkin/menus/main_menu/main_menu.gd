...
@onready var items: VBoxContainer = %container
@onready var background_animations: AnimationPlayer = %animation_player
...
func change_selection(amount: int = 0) -> void:
	var previous_item := items.get_child(selected) as MainMenuButton
	...
	var current_item := items.get_child(selected) as MainMenuButton
