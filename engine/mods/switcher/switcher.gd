...
@export var module_panel: PackedScene = load("uid://bqv4nkj1rm3om")

@export_group("References")
@export var root: Panel
@export var open_folder: Button
...
func load_modules_list() -> void:
	current_modules = Array(ResourceLoader.list_directory("res://mods"))
...
func module_path_filter(path: String) -> bool:
	...
	return ResourceLoader.exists("res://mods/%smodule.tscn" % path)
...
func recreate_panels() -> void:
	...
	for module: String in current_modules:
		...
		panel.get_node(^"ModuleName").text = module
		mod_container.add_child(panel)

	if Global.game_size.y >= 76.0:
		root.size.y = 44.0 + clampf(mod_container.get_minimum_size().y, 40.0, Global.game_size.y - 44.0)
	else:
		root.size.y = 76.0

	root.position.y = (Global.game_size.y - root.size.y) / 2.0
...
func change_selection(amount: int = 0) -> void:
	...
	for i: int in mod_container.get_child_count():
		var node := mod_container.get_child(i)
		var selected_panel := node.get_node(^"SelectedPanel")
		var module_name := node.get_node(^"ModuleName")
