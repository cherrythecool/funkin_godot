...
@export_file('*.tscn') var character_path: String = "uid://bu44d2he2dxm3":
	set(value):
...
@export var custom_camera_offset: Node2D
...
func adjust_character(input: Character, is_player: bool = false) -> void:
	...
	if custom_camera_offset:
		...
		input.camera_offset = custom_camera_offset
...
func instance_character(is_player: bool = false, insert: bool = true, adjust: bool = true) -> Character:
	...
	if adjust:
		...
		if custom_camera_offset:
			...
			instanced.camera_offset = custom_camera_offset
...
func make_camera_offset_node() -> void:
	...
	var offset := Marker2D.new()
	offset.name = &"CameraOffset"
	add_child(offset)
	offset.owner = get_parent()

	custom_camera_offset = offset

	if not is_instance_valid(character):
		return
	if not character.camera_offset:
		return

	offset.global_position = character.camera_offset.global_position
