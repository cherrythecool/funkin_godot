...
func load_assets() -> void:
	...
	if is_instance_valid(stage_parent):
		...
		if player:
			if stage.spawn_player:
				stage.spawn_player.adjust_character(player, true)
			if not player.starts_as_player:
				player.scale *= Vector2(-1.0, 1.0)

		if opponent:
			if stage.spawn_opponent:
				stage.spawn_opponent.adjust_character(opponent)
			if opponent.starts_as_player:
				opponent.scale *= Vector2(-1.0, 1.0)

		if spectator:
			if stage.spawn_spectator:
				stage.spawn_spectator.adjust_character(spectator)
