...
@export var camera: FunkinCamera2D
...
@export var timer_initial_focus: Timer
@export var timer_fade_out_delay: Timer
...
func _ready() -> void:
	...
	timer_initial_focus.timeout.connect(_on_initial_focus_timer_timeout)
	timer_fade_out_delay.timeout.connect(_on_fade_out_timer_timeout)

	timer_initial_focus.start()
...
func _on_fade_out_timer_timeout() -> void:
	...
	if character is CanvasGroup:
		tween.tween_property(character, ^"self_modulate:a", 0.0, 2.0)
	else:
