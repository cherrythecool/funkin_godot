class_name ModchartManager extends SubViewportContainer
...
var modifiers:Array[ModchartModifier] = [
	preload("uid://wi6uk0hbsiml").new(self), # accel
	preload("uid://dxicgix2rlo57").new(self), # beat
	preload("uid://xx6vbrj002o4").new(self), # drunk
	preload("uid://1grw6lvviqj").new(self), # opponent
	preload("uid://da10dict2e3ov").new(self) # rotate
]

var adapter:ModchartAdapter
var timeline:ModchartTimeline
var pool:ModchartObjectPool
...
func _process(delta:float) -> void:
	...
	# cleanup garabage objects
	for garbage in object_cache.keys():
		if !is_instance_valid(garbage):
			pool.add_to_pool(object_cache.get(garbage))
			object_cache.erase(garbage)
	...
	for receptor in adapter.get_receptors():
		...
		if !object_cache.has(receptor):
			receptor_3d = pool.get_object(adapter.get_note_sprite(receptor))
			receptors.add_child(receptor_3d)
			object_cache.set(receptor, receptor_3d)
			
		receptor.visible = false
		receptor_3d.update_properties()
		var pos:Vector3 = Vector3(receptor.global_position.x, receptor.global_position.y, 0)
		for mod in modifiers:
			pos = mod.get_position(pos, receptor, ObjectType.RECEPTOR, 0, adapter.get_receptor_direction(receptor), adapter.get_receptor_player(receptor))
	...
	for note in adapter.get_notes():
		...
		if !object_cache.has(note):
			note_3d = pool.get_object(adapter.get_note_sprite(note))
			notes.add_child(note_3d)
			object_cache.set(note, note_3d)
		
		note.visible = false
		note_3d.update_properties()
		var pos:Vector3 = Vector3(note.global_position.x, note.global_position.y, 0)
		for mod in modifiers:
			pos = mod.get_position(pos, note, ObjectType.NOTE, adapter.get_note_time(note), adapter.get_note_direction(note), adapter.get_receptor_player(note))
		note_3d.global_position = camera.project_position(Vector2(pos.x, pos.y), start_z_pos + pos.z)
