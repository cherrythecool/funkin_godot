...
func get_object(object: Node, _field: NoteField, column:int, player: int) -> void:
	...
	if tipsy_perc != 0:
		...
		object.position.y += tipsy_perc * (cos((time*((speed*1.2)+1.2) + column*((offset * 1.8)+1.8))) *112*.4)
	...
	if drunk_perc != 0:
		...
		object.position.x += drunk_perc * (cos(angle) * 112 * 0.5)
	
	if tip_z_perc != 0:
		var speed: float = submods.get("tip_z_speed").get_value(player)
		var offset: float = submods.get("tip_z_offset").get_value(player)
		object.z_index += tip_z_perc * (cos((time * ((speed * 1.2) + 1.2) + column * ((offset * 1.8) + 3.2))) * 0.15)
	...
	if bumpy_perc != 0:
		...
		object.z_index += (bumpy_perc * 40 * sin(angle))/250
