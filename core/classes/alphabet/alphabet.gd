...
@export var skin_override: AlphabetSkin = null:
	set(v):
		skin_override = v
		skin = skin_override
...
func _create_characters() -> void:
	...
	if not is_instance_valid(skin):
		if not text.strip_edges().is_empty():
			printerr("Need valid skin to create alphabet characters")
...
func _create_character(x: float, y: float, character: String) -> Array:
	...
	if is_instance_valid(skin_char):
		...
		if node.sprite_frames.has_animation(skin_char.animation + skin.suffix):
			node.animation = skin_char.animation + skin.suffix
		elif node.sprite_frames.has_animation(skin_char.animation.to_upper() + skin.suffix):
			node.animation = skin_char.animation.to_upper() + skin.suffix
		else:
			node.visible = false
	else:
		node.offset = Vector2.ZERO

		if node.sprite_frames.has_animation(character + skin.suffix):
			node.animation = character + skin.suffix
		elif node.sprite_frames.has_animation(character.to_upper() + skin.suffix):
			node.animation = character.to_upper() + skin.suffix
		else:
			node.visible = false
